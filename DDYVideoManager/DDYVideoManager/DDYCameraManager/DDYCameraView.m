...
- (UIButton *)filterButton {
    if (!_filterButton) {
        _filterButton = [self btnImg:@"filter" imgS:nil sel:@selector(handleFilter:)];
    }
...
// https://blog.csdn.net/czxghostyueqiu/article/details/46301445?locationNum=6&fps=1

- (NSMutableArray *)filterArray {
    if (!_filterArray) {
        _filterArray = [NSMutableArray array];
        [_filterArray addObject:@"NONONONONONOFilters"];
        [_filterArray addObject:@"CILinearToSRGBToneCurve"];
        ...
        [_filterArray addObject:@"CIPhotoEffectMono"];
        ...
        [_filterArray addObject:@"CICircularScreen"];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.filterButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.mas_equalTo(self.takeButton.mas_top).offset(-40);
            make.centerX.mas_equalTo(self.takeButton);
            make.width.height.mas_equalTo(30);
        }];
...
#pragma mark 滤镜按钮点击
- (void)handleFilter:(UIButton *)sender {
    static NSInteger filterNum = 0;
    filterNum ++;
    filterNum %= self.filterArray.count;
    if (self.filterBlock) self.filterBlock(self.filterArray[filterNum]);
    if (filterNum == 1) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            NSArray* filters =  [CIFilter filterNamesInCategory:kCICategoryBuiltIn];
            NSLog(@"filter number:%ld", filters.count);
