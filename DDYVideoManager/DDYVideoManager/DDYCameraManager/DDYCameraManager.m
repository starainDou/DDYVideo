...
- (GLKView *)previewView {
    if (!_previewView) {
        _previewView = [[GLKView alloc] initWithFrame:CGRectZero context:[[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2]];
...
- (AVAssetWriterInput *)assetVideoInput {
    if (!_assetVideoInput) {
        ...
        _assetVideoInput.transform = CGAffineTransformMakeRotation(M_PI_2);
    }
...
- (AVAssetWriterInputPixelBufferAdaptor *)pixelBufferAdaptor {
    if (!_pixelBufferAdaptor) {
        NSDictionary *attributes = @{(NSString *)kCVPixelBufferPixelFormatTypeKey:@(kCVPixelFormatType_32BGRA),
                                     (NSString *)kCVPixelBufferWidthKey:@([UIScreen mainScreen].bounds.size.height),
                                     (NSString *)kCVPixelBufferHeightKey:@([UIScreen mainScreen].bounds.size.width),
                                     (NSString *)kCVPixelFormatOpenGLESCompatibility:(NSNumber *)kCFBooleanTrue};
        _pixelBufferAdaptor = [[AVAssetWriterInputPixelBufferAdaptor alloc] initWithAssetWriterInput:self.assetVideoInput sourcePixelBufferAttributes:attributes];
    }
...
- (instancetype)initWithContainerView:(UIView *)view {
    if (self = [super init]) {
        [view insertSubview:self.previewView atIndex:0];
        ...
        if (imageDataSampleBuffer) {            
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (strongSelf.takeFinishBlock) {
                    strongSelf.takeFinishBlock([[[UIImage imageWithData:imageData] ddy_fixOrientation] ddy_FilterWithFilterName:self.filterName]);
                }
            });
        }
    }];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @autoreleasepool {
        if (captureOutput == self.videoOutput) {
            // 滤镜
            CIContext *previewContext = [CIContext contextWithEAGLContext:self.previewView.context options:@{kCIContextWorkingColorSpace:[NSNull null]}];
            CIImage *sampleImage = [CIImage imageWithCVPixelBuffer:CMSampleBufferGetImageBuffer(sampleBuffer)];
            CIImage *filterImage = filterCIImage(sampleImage, self.filterName);
            CMTime currentSampleTime = CMSampleBufferGetOutputPresentationTimeStamp(sampleBuffer);
            [self.previewView bindDrawable];
            if (self.previewView.context != [EAGLContext currentContext]) {
                [EAGLContext setCurrentContext:self.previewView.context];
                NSLog(@"self.previewView.context != [EAGLContext currentContext]");
            }
            ...
            if (filterImage) {
                [previewContext drawImage:filterImage inRect:CGRectMake(0, 0, self.previewView.drawableWidth, self.previewView.drawableHeight) fromRect:sampleImage.extent];
            }
            ...
            if (self.isRecording) {
                ...
                if (self.pixelBufferAdaptor.assetWriterInput.readyForMoreMediaData && self.assetWriter.status == AVAssetWriterStatusWriting) {
                    CVPixelBufferRef outputBuffer;
                    CVPixelBufferPoolCreatePixelBuffer(nil, self.pixelBufferAdaptor.pixelBufferPool, &outputBuffer);
                    [previewContext render:filterImage toCVPixelBuffer:outputBuffer bounds:filterImage.extent colorSpace:nil];
                    [self.pixelBufferAdaptor appendPixelBuffer:outputBuffer withPresentationTime:currentSampleTime];
                    CVPixelBufferRelease(outputBuffer);
                }
            }
        } else if (captureOutput == self.audioOutput){
            if (self.isRecording && self.assetAudioInput.readyForMoreMediaData && self.assetWriter.status == AVAssetWriterStatusWriting) {
...
static inline CIImage *filterCIImage(CIImage *cameraImage, NSString *filterName) {
    if (!filterName) return cameraImage;
    CIFilter *filter = [CIFilter filterWithName:filterName];
    if (!filter) return cameraImage;
    [filter setValue:cameraImage forKey:kCIInputImageKey];
    CIImage *currentImage = filter.outputImage;
    if (!currentImage || CGRectIsEmpty(currentImage.extent)) return cameraImage;
    return currentImage;
}
